...
namespace GeniusOneAi.WorkerPortal.Entities
{
    ...
    [ConnectionKey("Default"), Module("WorkerPortal"), TableName("[dbo].[WorkerInvoices]")]
    [DisplayName("My Invoices"), InstanceName("My Invoices")]
    [ReadPermission(PermissionKeys.MyInvoices)]
    [ModifyPermission(PermissionKeys.MyInvoices)]
    public sealed class MyInvoicesRow : Row<MyInvoicesRow.RowFields>, IIdRow, INameRow
    {
       [DisplayName("User Invoice Id"), Identity, IdProperty]
        ...
        [DisplayName("File Name"), Size(100)]
        public String FileName
        {
            get => fields.FileName[this];
            set => fields.FileName[this] = value;
        }
        public MyInvoicesRow()
        {
        }

        public MyInvoicesRow(RowFields fields)
            : base(fields)
        {
        }
